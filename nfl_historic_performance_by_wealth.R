...
setwd("~/Google Drive/NFL/nfl_rroi")
...
# Now calculate ROI by dividing Forbes2014Value / WinLossPerPost
df.nfl$ROI <- df.nfl$Forbes2014Value / df.nfl$WinLossPerPost
...
# ROI
df.nfl <- df.nfl[order(df.nfl$ROI),]
df.nfl$ROIRank <- c(1:32)
...
png("nfl_roi_sorted.png", width=480, height=480, units="px", res=72)
ggplot(df.nfl, aes(x=ROIRank, y=ROI, label=Team, fill=Conference))  + geom_bar(stat="identity")  + geom_text(aes(x=ROIRank, y=ROI, label=Team, size=1, angle=90)) + ylab("Historical Return on Investment (in $/Win%)") + xlab("Rank Order") + ggtitle("Historical ROI by Team")
dev.off()
